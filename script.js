...
// Ensure you load FFmpeg library via script tag in index.html
// Recommended: Use version 0.12.0 or later for better SharedArrayBuffer fallback
...
function processSubtitles(subtitleText, type, keepSegments) {
    ...
    let parsingState = '寻找提示标识符或时间'; // States: 寻找提示标识符或时间, 寻找时间, 寻找文本
    let cueHeaderLines = [];
    let cueTextLines = [];
        ...
        if (parsingState === '寻找提示标识符或时间') {
            ...
            if (timeMatch) {
             ...
             if (trimmedLine === '') {
                 ...
                 parsingState = '寻找提示标识符或时间'; // Ready for the next cue identifier/time
...
// --- Initial Load ---
// Add slight delay to ensure DOM elements are fully ready, especially on slower connections/devices
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM fully loaded and parsed");
    // Initial button state based on page load
    updateButtonState();
    // Now load FFmpeg
    loadFFmpeg();
});
